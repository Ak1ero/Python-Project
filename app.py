...
def initialize_database():
    try:
        ...
        # Check if recipes table has avg_rating column
        cur.execute("""
            SELECT COUNT(*) as count 
            FROM information_schema.columns 
            WHERE table_name = 'recipes' 
        ...
        if cur.fetchone()['count'] == 0:
            ...
            cur.execute("ALTER TABLE recipes ADD COLUMN avg_rating FLOAT DEFAULT 0")
            
        # Check if recipes table has ratings_count column
        cur.execute("""
            SELECT COUNT(*) as count 
            FROM information_schema.columns 
            WHERE table_name = 'recipes' 
        ...
        if cur.fetchone()['count'] == 0:
            # Add ratings_count column if it doesn't exist
            cur.execute("ALTER TABLE recipes ADD COLUMN ratings_count INT DEFAULT 0")
            
            # Initialize ratings counts and average ratings for existing recipes
            cur.execute("""
                UPDATE recipes r
                LEFT JOIN (
                    SELECT 
                    ...
                    FROM recipe_ratings
                    GROUP BY recipe_id
                ) as ratings ON r.id = ratings.recipe_id
                SET 
                    r.ratings_count = COALESCE(ratings.count, 0),
                    r.avg_rating = COALESCE(ratings.avg, 0)
            """)
    ...
    try:
        ...
        cur.execute("""
            SELECT 
                ...
                EXISTS(SELECT 1 FROM recipe_ratings WHERE recipe_id = r.id AND user_id = %s) as user_has_rated,
                r.comments_count,
                r.likes_count
            FROM recipes r
            JOIN users u ON r.user_id = u.id
...
def get_recipe_data(recipe_id):
    ...
    try:
        ...
        query = """
            ...
            WHERE r.id = %s
        """
        cur.execute(query, [session['user_id'], session['user_id'], session['user_id'], recipe_id])
        recipe = cur.fetchone()
        ...
        recipe_data = {
            ...
            'user_has_rated': bool(recipe['user_has_rated'])
        }
...
@app.route('/recipe/<int:recipe_id>/user-rating', methods=['GET'])
...
def rate_recipe(recipe_id):
    ...
    try:
        if existing_rating:
            # User has already rated this recipe - don't allow rating again
            # Just return the current rating information
            cur.execute("""
            ...
            return jsonify({
                ...
                'avg_rating': float(rating_stats['avg']),
                'ratings_count': rating_stats['count']
            })
        else:
            # Create new rating
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s)
            """, [user_id, recipe_id, rating_value, comment])
        
            # Update recipe's avg_rating and ratings_count
            cur.execute("""
                SELECT COUNT(*) as count, AVG(rating) as avg
                FROM recipe_ratings
                WHERE recipe_id = %s
            ...
            cur.execute("""
                UPDATE recipes
                SET avg_rating = %s, ratings_count = %s
                WHERE id = %s
            """, [rating_stats['avg'], rating_stats['count'], recipe_id])
